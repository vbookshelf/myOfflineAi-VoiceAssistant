...
- In MacOS, Ollama models run on the internal GPU (via Metal). That's the magic behind Ollama. Kokoro-onnx is supposed to run on the internal NPU (via CoreML), but in my testing (M4 Macbook Air) this wasn't happening. It was running on the CPU instead - but it was still fast.
